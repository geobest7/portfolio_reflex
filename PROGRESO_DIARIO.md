...
- `backend/app/models/experiencia.py` - Modelo Experiencia
  ...
  - Tecnologías (JSON), mostrar_en_web, orden
...
- Serialización con `from_attributes = True`
...
- 4 cursos/diplomas (Python, FastAPI, Git, Diploma Técnico)
- 1 experiencia (Prácticas en Tech Solutions)
...
- **Ordenamiento:** Campo `orden` para controlar visualización
...
- `pydantic-settings>=2.0.0` añadido a requirements.txt
...
4. Error handling
5. Opcional: GitHub API integration para repos dinámicos

### Commits de la Sesión
1. "Implementar backend FastAPI con CRUD completo"
2. "Actualizar documentación con backend implementado"

### Notas Técnicas
- **Soft delete:** Los registros no se eliminan físicamente, solo se marca `activo=False`
- **Multi-idioma:** Cada modelo tiene campos `*_es`, `*_en`, `*_it`, `*_ca`
- **JSON fields:** Tecnologías se guardan como JSON array
- **Fechas:** Usando `date` de Python para fecha_inicio/fecha_fin
- **Validación:** Pydantic valida automáticamente tipos y campos requeridos
